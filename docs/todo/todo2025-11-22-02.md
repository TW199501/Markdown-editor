...
- 本次完成：
  ...
  - 為多國語言支援做初步實作：在 `src/configure.ts` 中新增 `Lang.ZhTw = 'zh-TW'`，並更新 `src/i18n/i18n.ts` 的 `registerKeyset` 介面與實作，讓各 keyset 可以選擇性註冊 `zh-TW` 字典。
  - 依照既有 `en.json` / `ru.json` 的結構，在 `src/i18n/**` 下為多個常用 keyset 新增 `zh-TW.json`（如 `common`、`menubar`、`placeholder`、`search`、`widgets`、`yfm-*`、`gpt/*` 等），並在各自的 `index.ts` 以 `registerKeyset(KEYSET, {en, ru, 'zh-TW': zhTw})` 方式掛入。
  - 調整 demo Storybook 的 zh-TW 測試方式：最終回到透過 `.storybook/preview.ts` 中的 `withLang` decorator 由 toolbar 控制 `lang`（目前仍僅列出 `en` / `ru`），同時保留 library 端對 `configure({lang: 'zh-TW'})` 的正式支援，避免破壞官方預設 demo 行為。

### [驗證與結果]

- 目前狀態已從純規劃，進展到「已啟動 Storybook 並完成首次試用」：
  - 已在本機執行 `npm start` / `npm run storybook:start`，Storybook 能在 `http://localhost:8888` 正常載入各個 editor 範例（啟動過程中 Storybook 會互動詢問是否上傳匿名 crash reports，需在終端機回覆一次 Y/n 後即可穩定啟動）。
  ...
  - 新增 zh-TW 語系後，`tsc` 型別檢查通過，且 `registerKeyset` 對既有只含 `en` / `ru` 的 keyset 仍具相容性；目前 Storybook demo 仍透過 `withLang` 使用 `en` / `ru` 做展示，但 library 已可在外部專案中透過 `configure({lang: 'zh-TW'})` 切換到繁體中文 UI。
  - 目前尚未執行 `npm test` 或 `npm run build`，也尚未變更 editor 核心邏輯，僅針對 Storybook story 做極小幅語法修正與 i18n 結構延伸。
